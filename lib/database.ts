...
export const migrateData = () => {
  ...
  for (const prescription of prescriptionsDatabase) {
    try {
      prescriptionStatements.insert.run(
        prescription.id,
        prescription.userId,
  ...
  for (const pharmacy of pharmacies) {
    try {
      pharmacyStatements.insert.run(
        ...
        pharmacy.isOpen ? 1 : 0,
        pharmacy.openHours,
        pharmacy.isOnGuard ? 1 : 0,
        pharmacy.guardSchedule || null,
        pharmacy.emergencyPhone || null,
        pharmacy.location,
        JSON.stringify(pharmacy.services),
        JSON.stringify(pharmacy.specialties),
        JSON.stringify(pharmacy.paymentMethods),
        pharmacy.website || null,
        pharmacy.email || null,
        pharmacy.socialMedia ? JSON.stringify(pharmacy.socialMedia) : null,
        JSON.stringify(pharmacy.certifications),
        pharmacy.establishedYear,
        pharmacy.totalReviews,
        pharmacy.averageDeliveryTime,
        pharmacy.hasParking ? 1 : 0,
        pharmacy.isAccessible ? 1 : 0,
        JSON.stringify(pharmacy.languages),
        pharmacy.pharmacistName,
        pharmacy.pharmacistLicense,
        JSON.stringify(pharmacy.acceptedInsurance),
        0, // Posx default
        0  // Posy default
      );
    } catch (error) {
  ...
  for (const purchase of purchaseHistory) {
    try {
      purchaseHistoryStatements.insert.run(
        purchase.id,
        purchase.medicationName,
        purchase.medicationId,
        purchase.quantity,
        purchase.price,
        purchase.pharmacy.id,
        purchase.pharmacy.name,
        purchase.pharmacy.address,
        purchase.purchaseDate.toISOString(),
        purchase.status,
        purchase.prescriptionNumber || null,
        purchase.deliveryMethod,
        purchase.totalAmount,
        'cliente-1' // Use existing user
      );
    } catch (error) {
  ...
  for (const insurance of insuranceOptions) {
    try {
      insuranceStatements.insert.run(
        insurance.id,
        insurance.name,
        insurance.logo,
        insurance.coverage,
        ...
        JSON.stringify(insurance.pharmacyNetwork)
      );
    } catch (error) {
  ...
  for (const testUser of testUsers) {
    try {
      testUserStatements.insert.run(
        ...
        testUser.address,
        testUser.obraSocial,
        testUser.numeroAfiliado,
        testUser.birthDate,
        JSON.stringify(testUser.preferences),
        JSON.stringify(testUser.orderHistory)
      );
    } catch (error) {
